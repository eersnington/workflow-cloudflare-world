...
export interface WorkflowHandlerModules {
  flow: HandlerModule;
  step: HandlerModule;
  webhook: Partial<Record<WorkflowHttpMethod, WorkflowHandler>>;
}
...
export async function loadWorkflowHandlers(
  ...
  return createWorkflowHandlersFromModules({
    flow: flowModule,
    step: stepModule,
...
export function createWorkflowHandlersFromModules(
  modules: WorkflowHandlerModules
): WorkflowHandlers {
  const webhookHandlers: WorkflowHandlers['webhook'] = {};
  for (const method of SUPPORTED_WEBHOOK_METHODS) {
    const handler = modules.webhook?.[method];
    if (typeof handler === 'function') {
      webhookHandlers[method] = handler.bind(modules.webhook);
    }
  }

  return {
    step: bindPost(modules.step, HANDLER_FILENAMES.step),
    flow: bindPost(modules.flow, HANDLER_FILENAMES.flow),
